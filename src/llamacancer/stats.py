...
def fit_coxph_model(
    ...
    try:
        ...
        cph.fit(
            df_clean, duration_col=duration_col, event_col=event_col, formula=formula
        )
...
def run_logistic_regression(
    ...
    if len(df_clean) < len(covariates) + 5:  # Need more samples for logistic than Cox
        logging.warning(
            f"Insufficient data for logistic regression: {len(df_clean)} rows."
        )
        return None
            ...
            for covariate in covariates:
                if X[covariate].dtype == "object":
                    ...
                    dummies = pd.get_dummies(
                        X[covariate], prefix=covariate, drop_first=False
                    )
...
def run_group_comparison(
    ...
    if value_col not in data.columns or group_col not in data.columns:
        logging.error(
            f"Missing column(s) for group comparison: {value_col}, {group_col}"
        )
        return None
    is_continuous = (
        pd.api.types.is_numeric_dtype(data[value_col]) and data[value_col].nunique() > 5
    )
    stat_result = None
    if is_continuous:
        method = config.group_comparison_stat_continuous
        logging.info(
            f"Comparing continuous variable '{value_col}' by '{group_col}' using {method}"
        )
