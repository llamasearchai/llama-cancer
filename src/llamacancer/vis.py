    ...
    if group_col:
        ...
        for i, group_val in enumerate(groups):
            ...
            if mask.sum() == 0:
                continue
            label = (
                group_labels.get(group_val, group_val) if group_labels else group_val
            )
            kmf.fit(
                df.loc[mask, duration_col], df.loc[mask, event_col], label=str(label)
            )
...
def plot_volcano(
    ...
    if results_df is not None and not results_df.empty:
        ...
        if (
            "log2FoldChange" not in results_df.columns
            or "p_value" not in results_df.columns
        ):
            logging.error(
                "Volcano plot requires 'log2FoldChange' and 'p_value' columns."
            )
            return None
        ...
        if len(x_values) != len(p_values):
            logging.error(
                f"Length mismatch: x_values={len(x_values)}, p_values={len(p_values)}"
            )
            return None
        ...
        logging.error(
            "Either results_df or both x_values and p_values must be provided"
        )
        return None
    ...
    plot_df.loc[mask_up, "Significance"] = (
        f"Significant ({x_label} > {fc_threshold:.2f})"
    )

    mask_down = (plot_df[p_col] < sig_threshold) & (plot_df[x_col] < -fc_threshold)
    plot_df.loc[mask_down, "Significance"] = (
        f"Significant ({x_label} < {-fc_threshold:.2f})"
    )
    ...
    if label_col_to_use and n_labels > 0:
        ...
        try:
            ...
            for _, row in sig_df.iterrows():
                ax.text(
                    row[x_col], row["-log10(p)"], str(row[label_col_to_use]), fontsize=9
                )
