...
def setup_logging(level="INFO", log_file: Optional[str] = None):
    ...
    if log_file:
        try:
            ...
            print(
                f"Warning: Could not create log file handler for {log_file}. Error: {e}"
            )
...
def ensure_numeric(
    series: pd.Series, column_name: str, errors: str = "coerce"
) -> pd.Series:
...
def dichotomize_biomarker(
    ...
    if isinstance(cutoff_method, (int, float)):
        ...
        if cutoff_method.lower() == "median":
            ...
            logging.info(
                f"Dichotomizing '{series.name}' using median: {cutoff_val:.3f}"
            )
            ...
            logging.info(
                f"Categorizing '{series.name}' using quartiles: Q1={q1:.3f}, Q3={q3:.3f}"
            )
            groups = pd.cut(
                series,
            ...
            logging.warning(
                f"Unsupported cutoff method '{cutoff_method}' for {series.name}"
            )
            return None
    else:
        logging.warning(
            f"Invalid cutoff method type: {type(cutoff_method)} for {series.name}"
        )
        return None
