...
def load_clinical_data(config: config_dict.ConfigDict) -> Optional[pd.DataFrame]:
    ...
    for ep_name, ep_config in config.get("endpoints", {}).items():
        if ep_config.get("time_col") in df.columns:
            df[ep_config.time_col] = ensure_numeric(
                df[ep_config.time_col], ep_config.time_col
            )
    ...
    for name, filename in config.biomarker_files.items():
        ...
        if all_df is None:
            ...
            if len(overlap) > 0:
                ...
                all_df = all_df.join(
                    df, how="outer", lsuffix="_prev", rsuffix=f"_{name}"
                )
            else:
...
def merge_clinical_biomarkers(
    ...
    if len(overlap) > 0:
        warnings.warn(
            f"Overlapping columns: {list(overlap)}. Using suffixes.", UserWarning
        )
        merged = clinical_df.join(
            biomarker_df, how="left", lsuffix="_clin", rsuffix="_biom"
        )
    else:
