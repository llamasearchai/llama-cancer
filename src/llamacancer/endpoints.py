...
def check_endpoint_columns(df: pd.DataFrame, ep_config: dict, ep_name: str) -> bool:
    ...
    if response_col and response_col not in df.columns:
        logging.error(
            f"Endpoint '{ep_name}': Missing response column '{response_col}'."
        )
        valid = False

    if time_col in df.columns and not pd.api.types.is_numeric_dtype(df[time_col]):
        logging.warning(
            f"Endpoint '{ep_name}': Time column '{time_col}' is not numeric."
        )
    if status_col in df.columns and not pd.api.types.is_numeric_dtype(df[status_col]):
        try:
            ...
            logging.warning(
                f"Endpoint '{ep_name}': Status column '{status_col}' is not numeric."
            )
    return valid
...
def get_response_data(
    ...
    if not ep_config:
        logging.error(
            f"Response endpoint '{endpoint_name}' not defined in configuration."
        )
        return None
    if not check_endpoint_columns(df, ep_config, endpoint_name):
        return None
    ...
    if positive_values and isinstance(positive_values, list):
        ...
        logging.info(
            f"Binarized response column '{response_col}' using values: {positive_values}"
        )
