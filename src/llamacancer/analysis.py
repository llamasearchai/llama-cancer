...
def run_biomarker_associations(df: pd.DataFrame, config) -> Dict[str, Any]:
    ...
    for biomarker in config.biomarkers_to_analyze:
        ...
        # Response analysis
        if "response" in config.endpoints and hasattr(
            config.endpoints.response, "response_col"
        ):
    ...
    results["summary"] = {
        ...
        "significant_biomarkers": [
            ...
            if "cox_results" in v
            and v["cox_results"]["p_value"] < config.alpha_threshold
        ],
    }
...
def _dichotomize_biomarker(
    df: pd.DataFrame, biomarker: str, config
    ...
    if biomarker in config.biomarker_positive_categories:
        ...
        logging.info(
            f"Dichotomized '{biomarker}' based on positive category: {positive_value}"
        )
    ...
    # For continuous biomarkers with cutoffs
        ...
        if cutoff_value == "median":
            ...
            logging.info(
                f"Dichotomized '{biomarker}' at quartiles: Q1={q1:.3f}, Q3={q3:.3f}"
            )
            ...
            logging.warning(
                f"Unrecognized cutoff type for '{biomarker}': {cutoff_value}"
            )
            return None
...
def _analyze_survival_by_biomarker(
    ...
    plot_file = os.path.join(
        config.results_dir, f"km_{endpoint}_{biomarker}.{config.plot_format}"
    )
...
def _analyze_response_by_biomarker(
    ...
    try:
        ...
        plot_file = os.path.join(
            config.results_dir, f"response_{biomarker}.{config.plot_format}"
        )
        plt.tight_layout()
